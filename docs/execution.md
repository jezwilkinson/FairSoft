...
Linux
```
[fairsoft-env] $ spack view --dependencies true symlink -i (YOUR_SIMPATH) fairroot
```

macOS
```
[fairsoft-env] $ spack view --dependencies true -e libpng -e libjpeg-turbo -e libiconv -e sqlite symlink -i (YOUR_SIMPATH) fairroot
```
...
```
[fairsoft-env] $ spack load fairroot
```
